...
//				if(){grb.Sleep ();}
				if (collisiondetected && minHitDis < 1f) {
					if (!trb) {
						CollisionResponce (grb, collisionpoint, hitNormal);
		...
		public GameObject hitAt ()
		{
			...
			if (Physics.Raycast (ray, out hit, 100.0f)) {
				Transform prefab = hit.collider.transform;
...
//				print (hitdistance);
		...
		public void CollisionResponce (Rigidbody grb, Vector3 v, Vector3 hitNormal, float e = 0.3F)
		{
